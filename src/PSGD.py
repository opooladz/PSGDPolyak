...
class Newton:
    ...
    def __init__(self, params_with_grad, preconditioner_init_scale=None,
                 ...
                 exact_hessian_vector_product: bool = True, preconditioner_type="Newton",
                 distance_func=None, print_freq=100):
        # mutable members
        self.lr_params = lr_params
        if lr_preconditioner is None:
            if step_normalizer == '2nd':
    ...
    def step(self, closure):
        ...
        if self.iteration_counter % self.print_freq == 0:
            print(f"Iteration {self.iteration_counter}: loss = {loss.item()}")
            if self.distance_func is not None:
...
def run_psgd_algorithm(loss_fn, 
                      params, 
                      nb_gd_steps, 
                      gd_step_size,
                      distance_func=None, 
                      print_freq=100):
    # Define the PSGD Newton optimizer
    optimizer = Newton(
        params,
        lr_params=gd_step_size,
        momentum=0,
        preconditioner_init_scale=0.01,
        lr_preconditioner=1,
...
def run_psgd_algorithm(loss_fn, 
    ...
    for i in range(nb_gd_steps):
        optimizer.step(closure)
        # exponentially decay the learning rate based on nb gd steps
        optimizer.lr_params *= (0.9) ** (1 / (nb_gd_steps-1))
        optimizer.lr_preconditioner *= (0.9) ** (1 / (nb_gd_steps-1))
